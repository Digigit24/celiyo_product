...
interface BillPreviewTabProps {
  ...
  billItems: OPDBill['items'];
  billingData: BillingData;
  tenantData?: any;
  tenantSettings?: any;
  patientBills: OPDBill[];
  billsLoading: boolean;
  onPrint: () => void;
  onDownloadPDF: () => void;
}

export const BillPreviewTab = forwardRef<HTMLDivElement, BillPreviewTabProps>(
  (
    {
      visit,
      opdFormData,
        ...
        <Card>
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <CardDescription className="mt-1">
                  View all bills for this patient across all visits
                </CardDescription>
              </div>
          ...
          <CardContent>
            <DataTable
              ...
              getRowId={(row: OPDBill) => row.id}
              getRowLabel={(row: OPDBill) => row.bill_number || `Bill #${row.id}`}
              renderMobileCard={(row: OPDBill) => (
                <div className="space-y-2">
                  <div className="flex items-start justify-between">
                    <div>
                      <span className="font-mono font-medium">{row.bill_number}</span>
                      <p className="text-sm text-muted-foreground mt-1">
                        {row.bill_date && new Date(row.bill_date).toString() !== 'Invalid Date'
                          ? format(new Date(row.bill_date), 'dd MMM yyyy')
                          : 'N/A'}
                      </p>
                    </div>
                    <Badge
                  ...
                  <div className="flex justify-between items-center pt-2 border-t">
                    <span className="text-sm text-muted-foreground">Visit #{row.visit}</span>
                    <span className="font-semibold">₹{parseFloat(row.total_amount || '0').toFixed(2)}</span>
                  </div>
                </div>
              )}
              columns={[
...
BillPreviewTab.displayName = 'BillPreviewTab';
